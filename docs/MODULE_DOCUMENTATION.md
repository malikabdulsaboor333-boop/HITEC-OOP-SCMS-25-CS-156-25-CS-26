...
Classes: `Person`, `Student`, `Faculty`, `Staff`, `UndergradStudent`.
...
Relationships: `Student`, `Faculty`, and `Staff` inherit from `Person`. `UndergradStudent` inherits from `Student`.
