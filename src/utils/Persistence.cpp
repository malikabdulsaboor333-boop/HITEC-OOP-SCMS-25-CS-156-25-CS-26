...
const std::string undergradStudentsPath = dataPath + "undergrad_students.csv";
...
constexpr const char* undergradStudentHeader =
    "Name,CNIC,Age,Contact,RollNo,Department,CGPA,ProjectTitle,"
    "AcademicAdvisor";
...
bool CampusPersistence::ensureDataFiles() {
    const std::vector<std::pair<std::string, std::string>> files = {
        ...
        {undergradStudentsPath, undergradStudentHeader},
...
bool CampusPersistence::savePersons(
    ...
    std::ofstream staff;
    std::ofstream undergradStudents;
    if (!CsvUtils::openOutput(students, studentsPath, personHeader, append)
        ...
        || !CsvUtils::openOutput(
            undergradStudents,
            undergradStudentsPath,
            undergradStudentHeader,
            append)) {
        return false;
    }
    ...
    for (const std::unique_ptr<Person>& person : people) {
        ...
        if (const auto* undergraduate =
                dynamic_cast<const UndergradStudent*>(person.get())) {
            writePersonFields(undergradStudents, *undergraduate);
            undergradStudents << ','
                              << CsvUtils::escape(
                                     undergraduate->getRollNo())
                              << ','
                              << CsvUtils::escape(
                                     undergraduate->getDepartment())
                              << ',' << undergraduate->getCgpa() << ','
                              << CsvUtils::escape(
                                     undergraduate->getProjectTitle())
                              << ','
                              << CsvUtils::escape(
                                     undergraduate->getAcademicAdvisor())
                              << '\n';
...
std::vector<std::unique_ptr<Person>> CampusPersistence::loadPersons() {
    ...
    const auto loadUndergraduate = [&people](
                                       const std::vector<std::string>& fields) {
    ...
    readCsvRows(undergradStudentsPath, loadUndergraduate);
